...
- (NSDictionary *)toMap;
@end
@interface FLTBitrateMessage ()
+ (FLTBitrateMessage *)fromMap:(NSDictionary *)dict;
- (NSDictionary *)toMap;
@end
...
@implementation FLTBitrateMessage
+ (instancetype)makeWithTextureId:(NSNumber *)textureId
    bitrate:(NSNumber *)bitrate {
  FLTBitrateMessage* pigeonResult = [[FLTBitrateMessage alloc] init];
  pigeonResult.textureId = textureId;
...
+ (FLTBitrateMessage *)fromMap:(NSDictionary *)dict {
  ...
  pigeonResult.textureId = GetNullableObject(dict, @"textureId");
  NSAssert(pigeonResult.textureId != nil, @"");
  pigeonResult.bitrate = GetNullableObject(dict, @"bitrate");
  NSAssert(pigeonResult.bitrate != nil, @"");
  return pigeonResult;
}
- (NSDictionary *)toMap {
  return [NSDictionary dictionaryWithObjectsAndKeys:(self.textureId ? self.textureId : [NSNull null]), @"textureId", (self.bitrate ? self.bitrate : [NSNull null]), @"bitrate", nil];
}
@end
...
- (nullable id)readValueOfType:(UInt8)type 
{
  switch (type) {
    case 128:     
      return [FLTBitrateMessage fromMap:[self readValue]];
...
{
  if ([value isKindOfClass:[FLTBitrateMessage class]]) {
    [self writeByte:128];
...
void FLTAVFoundationVideoPlayerApiSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FLTAVFoundationVideoPlayerApi> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.AVFoundationVideoPlayerApi.setBitrate"
        binaryMessenger:binaryMessenger
        codec:FLTAVFoundationVideoPlayerApiGetCodec()        ];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(setBitrate:error:)], @"FLTAVFoundationVideoPlayerApi api (%@) doesn't respond to @selector(setBitrate:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FLTBitrateMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
        [api setBitrate:arg_msg error:&error];
        callback(wrapResult(nil, error));
      }];
    }
    else {
