...
class SKPaymentQueueWrapper {
  ...
  /// Adds a custom payment object to the payment queue.
  ///
  /// Most of the time you would use [addPaymentForProduct] for making a payment. This method is reserved for the necessity
  ...
  Future<void> addPayment(SKPaymentWrapper payment) async {
    assert(_observer != null, '[in_app_purchase]: Trying to add a payment without an observer. Observer must be set using `setTransactionObserver` before adding a payment `setTransactionObserver`. It is mandatory to set the observer at the moment when app launches.');
    Map requestMap = payment.toMap();
    requestMap['usePaymentObject'] = true;
  ...
  /// Adds a payment for a product to the default payment queue. If you would like to use a custom payment object
  /// to make a payment, use [addPayment]
  ///
  /// Use productIdentifier as the parameter will automatically create a payment object to process the payment for you.
  /// Prior to this call, you have to make sure At least one [SKTransactionObserverWrapper] should be added to the payment queue
  ...
  /// Each payment will generate a [SKPaymentTransactionWrapper]. After a payment is being added to the payment queue, the [SKTransactionObserverWrapper] is responsible to handle
  ...
  Future<void> addPaymentForProduct(String productIdentifier) async {
    assert(_observer != null, '[in_app_purchase]: Trying to add a payment without an observer. Observer must be set using `setTransactionObserver` before adding a payment `setTransactionObserver`. It is mandatory to set the observer at the moment when app launches.');
    await channel.invokeMethod(
      '-[InAppPurchasePlugin addPayment:result:]',
      ...
      case 'shouldAddStorePayment':
        {
          ...
          return await Future<void>(() {
            if (_observer.shouldAddStorePayment(
                payment: payment, product: product) == true) {
              SKPaymentQueueWrapper()
                  .addPaymentForProduct(product.productIdentifier);
            }
          });
        }
      default:
...
abstract class SKTransactionObserverWrapper {
  ...
  /// [addPaymentForProduct] with the product you get from this method.
