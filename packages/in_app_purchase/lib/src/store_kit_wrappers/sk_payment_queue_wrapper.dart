  ...
  Future<dynamic> _handleObserverCallbacks(MethodCall call) async {
    switch (call.method) {
      ...
      case 'restoreCompletedTransactions':
        {
          ...
          return await Future<void>(() {
            for (SKTransactionObserverWrapper observer in _observers) {
              observer.restoreCompletedTransactions(error: error);
            }
          });
        }
      ...
      case 'shouldAddStorePayment':
        {
          SKPaymentWrapper payment = SKPaymentWrapper.fromJson(call.arguments['payment']);
          SKProductWrapper product = SKProductWrapper.fromJson(call.arguments['product']);
          return await Future<bool>(() {
            for (SKTransactionObserverWrapper observer in _observers) {
              if (observer.shouldAddStorePayment(payment: payment, product: product)) {
                SKPaymentQueueWrapper().addPaymentForProduct(product.productIdentifier);
                return;
              }
            }
          });
