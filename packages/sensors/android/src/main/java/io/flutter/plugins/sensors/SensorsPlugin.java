...
public class SensorsPlugin implements FlutterPlugin {
  ...
  private EventChannel userAccelGravityChannel;
  ...
  private void setupEventChannels(Context context, BinaryMessenger messenger) {
    ...
    userAccelGravityChannel = new EventChannel(messenger, USER_ACCELEROMETER_GRAVITY_CHANNEL_NAME);
    final StreamHandlerImpl linearAccelerationAndGravityStreamHandler =
        new StreamHandlerImpl(
            (SensorManager) context.getSystemService(context.SENSOR_SERVICE),
            Sensor.TYPE_LINEAR_ACCELERATION,
            Sensor.TYPE_GRAVITY);
    userAccelGravityChannel.setStreamHandler(linearAccelerationAndGravityStreamHandler);
  ...
  private void teardownEventChannels() {
    ...
    userAccelGravityChannel.setStreamHandler(null);
