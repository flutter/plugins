...
- (BOOL)testIntegrationTest:(NSString **)testResult {
  IntegrationTestPlugin *integrationTestPlugin =
  ...
  [integrationTestPlugin
      setupChannels:flutterViewController.engine.binaryMessenger];
  ...
  NSDictionary<NSString *, NSString *> *testResults =
      integrationTestPlugin.testResults;
  ...
  if (!testPass && testResult) {
    *testResult = [NSString
        stringWithFormat:@"Detected failed integration test(s) %@ among %@",
