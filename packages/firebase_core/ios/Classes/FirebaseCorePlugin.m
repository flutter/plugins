...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  if ([@"FirebaseApp#configure" isEqualToString:call.method]) {
      ...
      FIROptions *options = [[FIROptions alloc] initWithGoogleAppID:optionsDictionary[@"googleAppID"] GCMSenderID:optionsDictionary[@"GCMSenderID"]];
      ...
      for (NSString *name in allApps) {
          ...
          [appsList addObject:@{ @"name": app.name,
                               @"options": app.options.dictionary
          }];
      }
