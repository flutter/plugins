...
public class FlutterBackgroundExecutor implements MethodCallHandler {
    ...
    public static void setCallbackDispatcher(Context context, long callbackHandle) {
        SharedPreferences prefs = context.getSharedPreferences(AlarmService.SHARED_PREFERENCES_KEY,
                0);
    ...
    public void onMethodCall(MethodCall call, Result result) {
        ...
        try {
            if (method.equals("AlarmService.initialized")) {
                ...
                // callback handles through the background method channel, and the Dart side will
                // execute
    ...
    /**
     * Starts running a background Dart isolate within a new {@link FlutterEngine} using a
     * previously
     * used entrypoint.
     *
     ...
     *
     * <ul>
     *   <li>The given callback must correspond to a registered Dart callback. If the handle does
     *   not
    ...
    public void startBackgroundIsolate(Context context) {
        if (!isRunning()) {
            SharedPreferences p = context.getSharedPreferences(AlarmService.SHARED_PREFERENCES_KEY,
                    0);
     ...
     *
     * <p>The given {@code intent} should contain a {@code long} extra called "callbackHandle",
     * which
    ...
    private void initializeMethodChannel(BinaryMessenger isolate) {
        ...
        //
        // This channel is also responsible for sending requests from Android to Dart to execute
        // Dart
