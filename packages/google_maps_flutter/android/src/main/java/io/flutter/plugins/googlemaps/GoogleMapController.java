    ...
    public void onMethodCall(MethodCall call, MethodChannel.Result result) {
        switch (call.method) {
            ...
            case "map#fromScreenLocation": {
                List<Integer> point = (List) call.argument("point");
            ...
            case "map#toScreenLocation": {
                List<Integer> point = (List) call.argument("position");
                LatLng p = Convert.toLatLng(point);
                Point resp = googleMap.getProjection().toScreenLocation(p);
                final Map<String, Object> arguments = new HashMap<>(2);
                arguments.put("point" , Convert.pointToJson(resp));
                result.success(arguments);
                break;
            }
            default:
