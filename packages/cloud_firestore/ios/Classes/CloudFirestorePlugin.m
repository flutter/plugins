...
static FlutterError *getFlutterError(NSError *error) {
  ...
  return [FlutterError
      errorWithCode:[NSString stringWithFormat:@"Error %ld", error.code]
...
static FIRQuery *getQuery(NSDictionary *arguments) {
  FIRQuery *query =
      [getFirestore(arguments) collectionWithPath:arguments[@"path"]];
  ...
  if (startAtDocument) {
    query =
        [query queryOrderedByFieldPath:FIRFieldPath.documentID descending:NO];
    query = [query
        queryStartingAtValues:getDocumentValues(startAtDocument, orderBy)];
  }
  ...
  if (startAfterDocument) {
    query =
        [query queryOrderedByFieldPath:FIRFieldPath.documentID descending:NO];
    query = [query queryStartingAfterValues:getDocumentValues(
                                                startAfterDocument, orderBy)];
  }
  ...
  if (endAtDocument) {
    query =
        [query queryOrderedByFieldPath:FIRFieldPath.documentID descending:NO];
    query =
        [query queryEndingAtValues:getDocumentValues(endAtDocument, orderBy)];
  }
  ...
  if (endBeforeDocument) {
    query =
        [query queryOrderedByFieldPath:FIRFieldPath.documentID descending:NO];
    query = [query
        queryEndingBeforeValues:getDocumentValues(endBeforeDocument, orderBy)];
  }
  return query;
}
...
static NSDictionary *parseQuerySnapshot(FIRQuerySnapshot *snapshot) {
  ...
  for (FIRDocumentChange *documentChange in snapshot.documentChanges) {
    ...
    [documentChanges addObject:@{
      ...
      @"oldIndex" :
          [NSNumber numberWithUnsignedInteger:documentChange.oldIndex],
      @"newIndex" :
          [NSNumber numberWithUnsignedInteger:documentChange.newIndex],
      @"metadata" : @{
        @"hasPendingWrites" :
...
- (id)readValueOfType:(UInt8)type {
  ...
  case TIMESTAMP: {
    ...
    return
        [[FIRTimestamp alloc] initWithSeconds:seconds nanoseconds:nanoseconds];
  }
  case GEO_POINT: {
    Float64 latitude;
    Float64 longitude;
  ...
  case DOCUMENT_REFERENCE: {
    ...
    FIRFirestore *firestore =
        [FIRFirestore firestoreForApp:[FIRApp appNamed:appName]];
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  ...
  FlutterMethodChannel *channel = [FlutterMethodChannel
      methodChannelWithName:@"plugins.flutter.io/cloud_firestore"
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _listeners = [NSMutableDictionary<NSNumber *, id<FIRListenerRegistration>>
            dictionary];
...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  void (^defaultCompletionBlock)(NSError *) =
      ^(NSError *error) { result(getFlutterError(error)); };
  if ([@"Firestore#runTransaction" isEqualToString:call.method]) {
    [getFirestore(
        call.arguments) runTransactionWithBlock:^id(FIRTransaction *transaction,
                                                    NSError **pError) {
        ...
        dispatch_semaphore_wait(
            ...
            dispatch_time(DISPATCH_TIME_NOW,
                          [transactionTimeout integerValue] * 1000000));
    ...
    dispatch_async(
        dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            FIRDocumentReference *document =
                getDocumentReference(call.arguments);
            FIRTransaction *transaction = self->transactions[transactionId];
            NSError *error = [[NSError alloc] init];

            FIRDocumentSnapshot *snapshot =
                [transaction getDocument:document error:&error];

            if (error != nil) {
              result([FlutterError
                  errorWithCode:[NSString stringWithFormat:@"%tu", [error code]]
                        message:[error localizedDescription]
                        details:nil]);
    ...
    FIRDocumentReference *document = getDocumentReference(call.arguments);
    [document updateData:call.arguments[@"data"]
              completion:defaultCompletionBlock];
    ...
    FIRDocumentReference *document = getDocumentReference(call.arguments);
    id<FIRListenerRegistration> listener =
        [document addSnapshotListener:^(FIRDocumentSnapshot *snapshot,
                                        NSError *_Nullable error) {
            if (snapshot == nil) {
              result(getFlutterError(error));
              return;
    ...
    if (![call.arguments[@"timestampsInSnapshotsEnabled"]
            isEqual:[NSNull null]]) {
