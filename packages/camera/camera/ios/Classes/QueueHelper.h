...
/// If caller site is already on the main queue, the block will be run synchronously. Otherwise, the block will be dispatch asynchronously to the main queue.
...
/// @returns true if the queue is on a certain queue specified by its queue-specific context data. false otherwise.
