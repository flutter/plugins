...
- (NSArray *)targetingInfoArrayForKey:(NSString *)key
  ...
  if (![value isKindOfClass:[NSArray class]]) {
    FLTLogWarning(@"targeting info %@: expected an array (MobileAd %@)", key,
                  self);
    return nil;
  }
...
- (NSString *)targetingInfoStringForKey:(NSString *)key
  ...
  if (![value isKindOfClass:[NSString class]]) {
    FLTLogWarning(@"targeting info %@: expected a string (MobileAd %@)", key,
                  self);
    return nil;
  }
  ...
  if ([stringValue length] == 0) {
    FLTLogWarning(
        @"targeting info %@: expected a non-empty string (MobileAd %@)", key,
        self);
    return nil;
  }
  return stringValue;
...
- (NSNumber *)targetingInfoBoolForKey:(NSString *)key
  ...
  if (![value isKindOfClass:[NSNumber class]]) {
    FLTLogWarning(@"targeting info %@: expected a boolean, (MobileAd %@)", key,
                  self);
    return nil;
  }
...
- (GADRequest *)createRequest {
  ...
  NSArray *testDevices =
      [self targetingInfoArrayForKey:@"testDevices" info:_targetingInfo];
  if (testDevices != nil) {
    request.testDevices = testDevices;
  }

  NSArray *keywords =
      [self targetingInfoArrayForKey:@"keywords" info:_targetingInfo];
  if (keywords != nil) {
    request.keywords = keywords;
  }

  NSString *contentURL =
      [self targetingInfoStringForKey:@"contentUrl" info:_targetingInfo];
  if (contentURL != nil) {
  ...
  if (gender != NULL) {
    if (![gender isKindOfClass:[NSNumber class]]) {
      FLTLogWarning(@"targeting info gender: expected an integer (MobileAd %@)",
                    self);
    } else {
  ...
  NSNumber *childDirected =
      [self targetingInfoBoolForKey:@"childDirected" info:_targetingInfo];
  if (childDirected != nil) {
    [request tagForChildDirectedTreatment:childDirected.boolValue];
  }

  NSString *requestAgent =
      [self targetingInfoStringForKey:@"requestAgent" info:_targetingInfo];
  if (requestAgent != nil) {
