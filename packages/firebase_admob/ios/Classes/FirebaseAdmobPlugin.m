...
- (void)callLoadAd:(MobileAd *)ad call:(FlutterMethodCall*)call result:(FlutterResult)result {
  ...
  if (unitId == nil || [unitId length] == 0) {
    NSString* message = [NSString stringWithFormat:@"a non-empty unitId was not provided for %@", ad];
...
- (void)callShowAd:(NSNumber *)mobileAdId call:(FlutterMethodCall*)call result:(FlutterResult)result {
  MobileAd* ad = [MobileAd getAdForId:mobileAdId];
  if (ad == nil) {
    NSString* message = [NSString stringWithFormat:@"show failed, the specified ad was not loaded id=%d", mobileAdId.intValue];
...
- (void)callDisposeAd:(NSNumber *)mobileAdId call:(FlutterMethodCall*)call result:(FlutterResult)result {
  MobileAd* ad = [MobileAd getAdForId:mobileAdId];
  if (ad == nil) {
    NSString* message = [NSString stringWithFormat:@"dispose failed, no ad exists for id=%d", mobileAdId.intValue];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  ...
  if (mobileAdId == nil) {
    NSString* message = @"all FirebaseAdMobPlugin method calls must specify an integer mobile ad id";
  ...
  if ([call.method isEqualToString:@"loadBannerAd"]) {
    ...
    [self callLoadAd:[InterstitialAd withId:mobileAdId channel:self.channel] call:call result:result];
