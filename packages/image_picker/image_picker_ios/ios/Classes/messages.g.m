...
- (NSDictionary *)toMap;
@end
@interface FLTIOSMediaSelectionTypeData ()
+ (FLTIOSMediaSelectionTypeData *)fromMap:(NSDictionary *)dict;
- (NSDictionary *)toMap;
@end
...
@implementation FLTIOSMediaSelectionTypeData
+ (instancetype)makeWithValue:(FLTIOSMediaSelectionType)value {
  FLTIOSMediaSelectionTypeData *pigeonResult = [[FLTIOSMediaSelectionTypeData alloc] init];
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    case 128:
      return [FLTIOSMediaSelectionTypeData fromMap:[self readValue]];
...
- (void)writeValue:(id)value {
  if ([value isKindOfClass:[FLTIOSMediaSelectionTypeData class]]) {
    [self writeByte:128];
  ...
  }
  {
    ...
    if (api) {
      NSCAssert([api respondsToSelector:@selector
                     (pickMediaWithMaxSize:quality:allowMultiple:allowedTypes:completion:)],
                @"FLTImagePickerApi api (%@) doesn't respond to "
                @"@selector(pickMediaWithMaxSize:quality:allowMultiple:allowedTypes:completion:)",
                api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FLTMaxSize *arg_maxSize = GetNullableObjectAtIndex(args, 0);
        NSNumber *arg_imageQuality = GetNullableObjectAtIndex(args, 1);
        NSNumber *arg_allowMultiple = GetNullableObjectAtIndex(args, 2);
        NSArray<FLTIOSMediaSelectionTypeData *> *arg_allowedTypes =
            GetNullableObjectAtIndex(args, 3);
        [api pickMediaWithMaxSize:arg_maxSize
                          quality:arg_imageQuality
                    allowMultiple:arg_allowMultiple
                     allowedTypes:arg_allowedTypes
                       completion:^(NSArray<NSString *> *_Nullable output,
                                    FlutterError *_Nullable error) {
                         callback(wrapResult(output, error));
                       }];
