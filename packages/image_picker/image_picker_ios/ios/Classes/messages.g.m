...
- (NSDictionary *)toMap;
@end
@interface FLTIOSRetrieveTypeData ()
+ (FLTIOSRetrieveTypeData *)fromMap:(NSDictionary *)dict;
- (NSDictionary *)toMap;
@end
...
@implementation FLTIOSRetrieveTypeData
+ (instancetype)makeWithValue:(FLTIOSRetrieveType)value {
  FLTIOSRetrieveTypeData *pigeonResult = [[FLTIOSRetrieveTypeData alloc] init];
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    case 128:
      return [FLTIOSRetrieveTypeData fromMap:[self readValue]];
...
- (void)writeValue:(id)value {
  if ([value isKindOfClass:[FLTIOSRetrieveTypeData class]]) {
    [self writeByte:128];
...
void FLTImagePickerApiSetup(id<FlutterBinaryMessenger> binaryMessenger,
  ...
  {
    FlutterBasicMessageChannel *channel = [[FlutterBasicMessageChannel alloc]
           initWithName:@"dev.flutter.pigeon.ImagePickerApi.pickRecentMedia"
        binaryMessenger:binaryMessenger
                  codec:FLTImagePickerApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector
                     (pickRecentMediaWithType:maxSize:quality:limit:completion:)],
                @"FLTImagePickerApi api (%@) doesn't respond to "
                @"@selector(pickRecentMediaWithType:maxSize:quality:limit:completion:)",
                api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FLTIOSRetrieveTypeData *arg_type = GetNullableObjectAtIndex(args, 0);
        FLTMaxSize *arg_maxSize = GetNullableObjectAtIndex(args, 1);
        NSNumber *arg_imageQuality = GetNullableObjectAtIndex(args, 2);
        NSNumber *arg_limit = GetNullableObjectAtIndex(args, 3);
        [api pickRecentMediaWithType:arg_type
                             maxSize:arg_maxSize
                             quality:arg_imageQuality
                               limit:arg_limit
                          completion:^(NSArray<NSString *> *_Nullable output,
