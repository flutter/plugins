...
public class FlutterWebViewTest {
    ...
    private FlutterWebView initFlutterWebView() {
        try (MockedStatic<FlutterWebView> mockedStaticFlutterWebView = mockStatic(FlutterWebView.class)) {

            mockedStaticFlutterWebView.when(new MockedStatic.Verification() {
                @Override
                public void apply() throws Throwable {
                    FlutterWebView.createWebView(ArgumentMatchers.<WebViewBuilder>any(), ArgumentMatchers.<String, Object>anyMap(), ArgumentMatchers.<WebChromeClient>any());
                }
            }).thenReturn(mockWebView);

            return new FlutterWebView(mockContext, mockChannel, createParameterMap(false), mockView, mockDisplayListenerProxy);
        }
    }
}
