...
class FlutterWebViewClient {
    ...
    private WebResourceResponse shouldInterceptRequest(WebView view, WebResourceRequest request) {
        ...
        String currentHostUrl = this.currentHostUrl;
        if (ContentBlocker.INSTANCE.isReady() && !request.isForMainFrame()) {
            ContentType contentType = ContentType.guessContentType(request);
            Uri hostedUrl = currentHostUrl != null ? Uri.parse(currentHostUrl) : Uri.parse("");
            BlockResult result = ContentBlocker.INSTANCE.shouldBlock(hostedUrl, requestUri, contentType);
            if (result == BlockResult.BLOCK) {
                Log.d(TAG, "Blocked  host: " + hostedUrl +", request: " + requestUri + ",  type: "+ contentType);
