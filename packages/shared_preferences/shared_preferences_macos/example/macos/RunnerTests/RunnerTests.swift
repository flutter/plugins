...
class RunnerTests: XCTestCase {
  func testHandlesCommitNoOp() throws {
    ...
    var called = false;
    plugin.handle(call, result:  { (result: Any?) -> Void in
      called = true
  ...
  func testSetAndGet() throws {
    let plugin = SharedPreferencesPlugin()
    let setCall = FlutterMethodCall(methodName: "setInt", arguments: [
      "key": "flutter.foo",
      "value": 42,
    ])
    ...
    var value: Int?
    plugin.handle(FlutterMethodCall(methodName: "getAll", arguments: nil), result: { (result: Any?) -> Void in
      if let prefs = result as? Dictionary<String, Any> {
  ...
  func testClear() throws {
    ...
    var value: Int?
    plugin.handle(getCall, result:  { (result: Any?) -> Void in
      if let prefs = result as? Dictionary<String, Any> {
        value = prefs["flutter.foo"] as? Int
      }
    })
    ...
    // Clear the value.
    plugin.handle(FlutterMethodCall(methodName: "clear", arguments: nil), result:  { (result: Any?) -> Void in
      XCTAssert(result as? Bool == true)
    })
