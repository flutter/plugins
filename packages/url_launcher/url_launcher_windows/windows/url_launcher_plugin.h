...
class UrlLauncherPlugin : public flutter::Plugin {
 ...
 private:
  // Reports via |result| whether or not the given URL has a registered handler.
  void CanLaunchUrl(const std::string& url,
                    std::unique_ptr<flutter::MethodResult<>> result);

  // Attempts to launch the given URL, and reports the outcome via |result|.
