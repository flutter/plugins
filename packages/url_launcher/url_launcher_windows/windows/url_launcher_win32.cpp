...
void OpenLink(
  ...
  int status = static_cast<int>(reinterpret_cast<INT_PTR>(
      ::ShellExecute(nullptr, TEXT("open"), url_wide.c_str(), nullptr, nullptr,
                     SW_SHOWNORMAL)));

  if (status <= 32) {
    std::ostringstream error_message;
    error_message << "Failed to open " << url_string
...
void CanLaunch(
  ...
  if (separator_location != std::string::npos) {
    std::wstring scheme =
        Utf16FromUtf8(url_string.substr(0, separator_location));
    HKEY key = nullptr;
