...
void main() {
  group('$Firestore', () {
    ...
    final CollectionsReference collectionsReference = firestore.collection('foo');

    setUp(() async {
      channel.setMockMethodCallHandler((MethodCall methodCall) async {
        log.add(methodCall);
        switch(methodCall.method) {
          case 'CollectionsReference#listen':
            return mockHandleId++;
          case 'CollectionsReference#set':
            return true;
          default:
            return null;
        }
    ...
    test('listen', () async {
      collectionsReference.listen(() {});
      expect(
        log,
        equals(<MethodCall>[
          const MethodCall('CollectionsReference#listen')
        ])
      );
    });

    test('set', () async {
      collectionsReference.set(<String, String>{'fooKey': 'barValue');
      expect(
        log,
        equals(<MethodCall>[
          const MethodCall(
            'CollectionsReference#set',
            <String, String> {
              'fooKey': 'barValue'
            }
          )
