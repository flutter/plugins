...
if [[ "${BRANCH_NAME}" == "master" ]]; then
  ...
  (cd "$REPO_DIR" && pub.bat global run flutter_plugin_tools "${ACTIONS[@]}" ${PLUGIN_SHARDING[@]})
else
  ...
  if [[ "$CHANGED_PACKAGES" == "" ]]; then
    ...
    (cd "$REPO_DIR" && pub.bat global run flutter_plugin_tools "${ACTIONS[@]}" --plugins="$CHANGED_PACKAGES" ${PLUGIN_SHARDING[@]})
    echo "Running version check for changed packages"
    (cd "$REPO_DIR" && pub.bat global run flutter_plugin_tools version-check --base_sha="$(get_branch_base_sha)")
  fi
fi
